...
export default {
  ...
  mounted() {
    ...
    // 订阅消息
    this.checkObjId = pubsub.subscribe('checkObj', (_, id) => this.checkObj(id))
    this.deleteTodoObjId = pubsub.subscribe('deleteTodoObj', (_, id) => this.deleteTodoObj(id))
  },
  beforeDestroy() {
    // this.$bus.$off('checkObj');
    // this.$bus.$off('deleteTodoObj');
    pubsub.unsubscribe(this.checkObjId);
    pubsub.unsubscribe(this.deleteTodoObjId);
  },
  watch: {
    todoList: {
